...
class MainActivity : AppCompatActivity() {
    ...
    // AIzaSyCTXOgpLY2SlhLsNRttPmow0NXodHz0ggw
    ...
    fun setMyLocation(location:Location) {
        ...
        // 마커의 이미지를 설정해준다.
        val markerBitmap = BitmapDescriptorFactory.fromResource(R.drawable.png)
    ...
    fun showPlaceDialog(){
        val materialAlertDialogBuilder = MaterialAlertDialogBuilder(this@MainActivity)
        materialAlertDialogBuilder.setTitle("장소 종류 선택(영어 ㅈㅅ)")
        ...
        if(userLocation != null){
            CoroutineScope(Dispatchers.Main).launch {
                val work1 = async(Dispatchers.IO){
                    ...
                    val radius = "1000"
                    val language = "ko"
                    val type = dialogData[position] // 문서에 지정된 타입을 넣어준다 ex) bank, store 이런식이라 영어로 문자열데이터를 넣은거
                    val key = "AIzaSyCTXOgpLY2SlhLsNRttPmow0NXodHz0ggw"
            ...
            CoroutineScope(Dispatchers.Main).launch {
                ...
                if(markerBitmap == null){
                    val work1 = async(Dispatchers.IO){
                        ...
                        // 이미지 데이터를 받아온다.
                        BitmapFactory.decodeStream(inputStream) // await 이 받는 리턴부분
                    }
                    markerBitmap = work1.await() // 스트림을 받는다
                }
                // 가져온 장소의 수 만큼 반복한다.
