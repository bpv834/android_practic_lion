...
class BottomSheetPlannerListModify(val plannerFragment: PlannerFragment) : BottomSheetDialogFragment() {
...
### 데이터 베이스와 연동시키를 VO 클래스를 작성해준다.

[database/PlannerVO.kt]
```kt
import androidx.room.Entity
import androidx.room.PrimaryKey
...
### StudentDataBase에 PlannerVO, 와 PlannerDao 를 설정한다

[database/StudentDataBase.kt]
```kt

@Database(entities = [StudentVO::class, PlannerVO::class], version = 1, exportSchema = true)
abstract class StudentDataBase : RoomDatabase(){
    // dao
    abstract fun studentDao() : StudentDao
...
### 학사 일정 등록을 위한 BottomSheet Fragment를 만들어준다.

[fragment/BottomSheetPlannerAddFragment.kt]
...
<LinearLayout xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    android:paddingStart="10dp"
    android:paddingEnd="10dp"
    tools:context=".fragment.BottomSheetPlannerAddFragment">
    ...
    <com.google.android.material.textfield.TextInputLayout
        android:id="@+id/textFieldBottomSheetPlannerAddContent"
        android:layout_width="match_parent"
        android:layout_height="wrap_content"
        android:hint="학사 일정"
        app:endIconMode="clear_text">
    ...
    <Button
        android:id="@+id/buttonBottomSheetPlannerAddDate"
        style="@style/Widget.Material3.Button.OutlinedButton"
        android:layout_width="match_parent"
        android:layout_height="wrap_content"
        android:layout_marginTop="10dp"
    ...
    <Button
        android:id="@+id/buttonBottomSheetPlannerAddSubmit"
...
### 학사 일정 추가 bottom sheet를 띄우는 메서드를 만들어준다.
...
### 툴바의 메뉴를 누르면 BottomSheet가 보이도록 한다.
[fragment/PlannerFragment.kt - settingToolbar()]
```kt
                    R.id.planner_menu_add -> {
                        // 학사 일정 BottomSheet을 띄운다.
...
[fragment/BottomSheetPlannerAddFragment.kt - onCreateView()]
```kt
...
[fragment/BottomSheetPlannerAddFragment.kt - settingButton()]
```kt
...
### PlannerRepository를 만들어준다.

[repository/PlannerRepository.kt]
...
[fragment/PlannerListFragment.kt - RecyclerViewPlannerListAdapter]
```kt
        override fun getItemCount(): Int {
...
### 데이터를 읽어와 Recyclerview를 갱신하는 메서드를 만들어준다.
...
### Calender나 Recyclerview를 갱신하는 메서드를 구현다.
[fragment/PlannerFragment.kt]
```kt
...
### BottomSheet를 띄우는 메서드 호출 부분을 수정한다.
...
### 데이터를 가져와 입력 요소들에 설정해주는 메서드를 구현한다.

[fragment/BottomSheetPlannerListModify.kt - gettingPlannerData()]
```kt
...
### 메서드를 호출한다.
[fragment/BottomSheetPlannerListModify.kt - onCreateView()]
```kt
